...
void Net<Ttype, Ptype, RunType>::init(graph::Graph<Ttype, Ptype>& graph, \
    ...
    for (auto & node_name : node_names_in_exec_order) {
        ...
        auto* op_pointer = CalibratorOp(node_ptr->get_op_name(), node_ptr->name(), _calibrator_parser);
...
void Net<Ttype, Ptype, RunType>::init(graph::Graph<Ttype, Ptype>& graph) {
    ...
    for (auto & node_name : node_names_in_exec_order) {
        ...
        if ((std::string::npos != (node_ptr->get_op_name()).find("Conv")
                ...
                && std::string::npos == (node_ptr->get_op_name()).find("ConvUnpadding")) {
            ...
            _op_param.push_back(buf);
                   ...
                   && std::string::npos == (node_ptr->get_op_name()).find("TopK")) {
        ...
        if (std::is_same<Ttype, NV>::value) {
            if (node_ptr->get_op_name() == "ConvBatchnormScale" ||
                ...
                if ((group_val == 1) && (k_w == 3 && k_h == 3 && dil_h == 1 && dil_w == 1)) {
                    ...
                    auto* op_pointer = CalibratorOp("Sass" + node_ptr->get_op_name(), node_ptr->name(),
                                                    _calibrator_parser);
    ...
    for (auto & executer : _exec_funcs) {
        ...
        for (auto out : executer.outs) {
            LOG(INFO) <<  executer.name << "  |-- out tensor avg " << tensor_mean_value(*out);
        }
    ...
    for (auto & executer : _exec_funcs) {
        ...
        for (auto out : executer.outs) {
            LOG(INFO) << "    \\ out shape (" << out->valid_shape() << ") "
                      << "executer name:" << executer.name << " avg: " << tensor_mean_value(*out);
        }
...
void Net<Ttype, Ptype, RunType>::execute_stop_at_node(std::string node_name) {
    ...
    for (int i = 0; i < _suspended_point; i++) {
        ...
        for (auto out : executer.outs) {
            LOG(INFO) << "    |-- out tensor avg " << tensor_mean_value(*out);
        }
