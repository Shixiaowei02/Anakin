...
Status ConvUnpaddingPaddingHelper<Ttype, Ptype>::Init(OpContext<Ttype>& ctx,
    ...
    SABER_CHECK(_funcs_conv_upadding_padding.init(ins, outs, _param_conv_upadding_padding, SPECIFY,
                SABER_IMPL, ctx));
    return Status::OK();
}

template<typename Ttype, Precision Ptype>
Status ConvUnpaddingPaddingHelper<Ttype, Ptype>::InferShape(const std::vector<Tensor4dPtr<Ttype>>&
        ins,
        std::vector<Tensor4dPtr<Ttype>>& outs) {
    SABER_CHECK(_funcs_conv_upadding_padding.compute_output_shape(ins, outs,
                _param_conv_upadding_padding));
    return Status::OK();
}
