...
template<typename Ttype, Precision Ptype>
Status InterpHelper<Ttype, Ptype>::Init(OpContext<Ttype>& ctx,
                                        const std::vector<Tensor4dPtr<Ttype>>& ins,
                                        std::vector<Tensor4dPtr<Ttype>>& outs) {
    // get interp param
...
Status InterpHelper<Ttype, Ptype>::InferShape(const std::vector<Tensor4dPtr<Ttype>>& ins,
    ...
    LOG(INFO) << "shapeIN" << shape_in[0] << " " << shape_in[1] << " " << shape_in[2] << " " <<
              shape_in[3];

    for (int i = 1; i < outs.size(); ++i) {
        outs[i]->set_shape(outs[0]->shape());
        LOG(INFO) << "shape" << i << ":" << shape_n[0] << " " << shape_n[1] << " " << shape_n[2] << " " <<
                  shape_n[3];
    }
